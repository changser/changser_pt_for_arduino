...
6.6、何时用PT_INIT(&pt)的形式，
	 ...
	 而PT_WAIT_SIGNAL(pt)一般用于某任务内，这个任务定义时，是需要关联TCB的，如PT_THREAD(producer(struct pt *pt)。所以，
	 是将指针型参数pt传递过来的，直接写PT_WAIT_SIGNAL(pt)就可以了。
	 那么，PT_WAIT_SIGNAL(pt)呢？实际如果定义一个pt型结构thread3，是任务3，如果在任务1里头发信号，则PT_WAIT_SIGNAL(&thread3)
...
7.2 pt-timer.h定义
  PT_WAIT_MS(pt,time)    即PT_TIMER_DELAY(pt,time) 功能：pt所在任务等time时间，单位ms，转移控制权
  PT_WAIT_US(pt,time)    即PT_TIMER_MICRODELAY(pt,time) 功能：pt所在任务等time时间，单位us，转移控制权
